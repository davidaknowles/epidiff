...
require(scales)
png("poised_tomek.png",width=3000,height=3000)
...
for (i in 1:n) for (j in 1:n)
{
	...
	load(paste(path,cell1,"_H3K27me3_peaks.RData",sep=""))
        peaks1=peaks/sum(peaks)
	load(paste(path,cell2,"_H3K27me3_peaks.RData",sep=""))
	peaks2=peaks/sum(peaks)
	rqs[cell1,cell2]=summary(lm(peaks1~peaks2))$r.squared
	plot( peaks1, peaks2 , xlab=cell1, ylab=cell2, main="", log="xy", col=alpha("black",.05) )
}
dev.off()
save(rqs,file="rsquares_poised_tomek.RData") 
