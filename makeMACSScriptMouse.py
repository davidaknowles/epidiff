def makeMACSScript(fileNameListFileName, path, outpath, ChIPinputFileName, modeFileName, outputFileName):
	...
	for line in fileNameListFile:
		...
		if mode == "Broad":
			# Use broad peaks
			outputFile.write("/opt/python/bin/python2.7 /srv/gs1/projects/snyder/yxl/old_home/yxl/src/MACS-2.0.9/bin/macs2 -t " + fileName + " -c " + ChIPInput + " -n " + outpath + experimentName + " -f BED -g mm -q 1e-4 --broad" + "\n")
		else:
			outputFile.write("/opt/python/bin/python2.7 /srv/gs1/projects/snyder/yxl/old_home/yxl/src/MACS-2.0.9/bin/macs2 -t " + fileName + " -c " + ChIPInput + " -n " + outpath + experimentName + " -f BED -g mm -q 1e-5" + "\n")
